...
byte TM16xx::getClockPin() {
    return this->clockPin;
}
byte TM16xx::getDataPin() {
    return this->dataPin;
}

void TM16xx::clearDisplay() {           // Clear all data registers. The number of registers depends on the chip.
...
void TM16xx::setSegments(byte segments, byte position) {  // set 8 leds on common grd as specified
    if (position < _maxDisplays)
        sendData(position, segments);
    // sendData(TM16XX_CMD_ADDRESS | position, segments);
}

void TM16xx::setSegments16(uint16_t segments, byte position) {  // Some modules support more than 8 segments (e.g. 10 max for TM1638)
...
void TM16xx::sendAsciiChar(byte pos, char c, boolean fDot) {  // Method to send an Ascii character to the display
...
void TM16xx::setDisplay(const byte values[], byte size) {  // send an array of values to the display
...
byte TM16xx::getNumDigits() {  // get the number of digits used (needed by TM16xxDisplay to combine modules)
    return (digits);
}

// key-scanning method, implemented in chip specific derived class
uint32_t TM16xx::getButtons() {  // return state of up to 32 keys.
    return (0);
}
...
void TM16xx::bitDelay() {  // if needed derived classes can add a delay (eg. for TM1637)
...
void TM16xx::start() {  // if needed derived classes can use different patterns to start a command (eg. for TM1637)
    digitalWrite(strobePin, LOW);
    bitDelay();
}

void TM16xx::stop() {  // if needed derived classes can use different patterns to stop a command (eg. for TM1637)
    digitalWrite(strobePin, HIGH);
    bitDelay();
}
